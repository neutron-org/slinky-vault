...
pub fn get_prices(
    ...
    if token_a.denom.eq(&config.maxbtc_denom) {
        token_0_price = token_0_price.checked_mul(config.maxbtc_redemption_rate)?;
    ...
    if token_b.denom.eq(&config.maxbtc_denom) {
        token_1_price = token_1_price.checked_mul(config.maxbtc_redemption_rate)?;
...
pub fn query_dual_redemption_rates(deps: Deps, env: Env) -> ContractResult<DualRedemptionRateResponse> {
    ...
    let maxbtc_exchange_rate = PrecDec::to_string(&config.maxbtc_redemption_rate);
    let maxbtc_exchange_rate_decimal = Decimal::from_str(&maxbtc_exchange_rate).map_err(|_| ContractError::DecimalConversionError)?;
    ...
    Ok(DualRedemptionRateResponse {
        maxbtc_redemption_rate: maxbtc_exchange_rate_decimal,
