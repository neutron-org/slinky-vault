...
fn test_get_deposit_data(
    ...
    let prices = CombinedPriceResponse {
        token_0_price: Decimal::from_str(token_0_price).unwrap(),
        token_1_price: Decimal::from_str(token_1_price).unwrap(),
        price_0_to_1: Decimal::from_str(price_0_to_1).unwrap(),
    };

    get_deposit_data(
        Uint128::new(total_available_0),
        Uint128::new(total_available_1),
        Uint128::new(expected_amount_0),
        Uint128::new(expected_amount_1),
        tick_index,
        fee,
        &prices,
