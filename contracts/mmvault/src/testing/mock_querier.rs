...
use cosmwasm_std::{
    from_json, to_binary, BalanceResponse, BankQuery, Binary, Coin, ContractResult, Empty,
...
impl MockQuerier {
    ...
    pub fn set_contract_balance(&mut self, contract_addr: &str, balances: Vec<Coin>) {
        self.contract_balances
            .insert(contract_addr.to_string(), balances);
    }
    ...
    pub fn set_supply(&mut self, denom: &str, amount: u128) {
        self.token_supply
            .insert(denom.to_string(), Uint128::new(amount));
    }
    ...
    fn raw_query(&self, bin_request: &[u8]) -> QuerierResult {
        ...
        match request {
            QueryRequest::Wasm(WasmQuery::Smart { contract_addr, msg }) => {
                ...
                // Handle dex deposit queries
                    ...
                    if let Some(deposits) = &self.deposits {
                        return SystemResult::Ok(ContractResult::Ok(to_binary(deposits).unwrap()));
                    }
                }
                // Handle balance queries
                    ...
                    if let Some(balances) = self.contract_balances.get(&contract_addr) {
                        return SystemResult::Ok(ContractResult::Ok(to_binary(&balances).unwrap()));
                    }
                }
            ...
            QueryRequest::Grpc(grpc_query) => {
                ...
                if path.contains("UserDepositsAll") {
                    if let Some(deposits) = &self.deposits {
                        ...
                        return SystemResult::Ok(ContractResult::Ok(Binary::from(encoded)));
                    }
                }
            ...
            QueryRequest::Bank(BankQuery::Supply { denom }) => {
                ...
                let supply_response = cosmwasm_std::SupplyResponse::new(Coin {
                    denom: denom.clone(),
                    amount,
                });
